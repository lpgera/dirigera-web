...
// Type for the device image configuration
// Simple mapping: deviceId -> imageFilename
type DeviceImageConfig = Record<string, string>;

/**
 * Custom hook to load and manage device image configuration
 * Fetches the config from /device-images.config.json at runtime
 * Returns a function to get image path for a given device ID
 */
export const useDeviceImages = () => {
  ...
  /**
   * Get the image path for a device ID
   * Returns undefined if no image is configured for this device
   */
